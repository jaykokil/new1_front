...
function MachineStatusCard({ onBarcodeDetected, onWeightDetected, currentWeight, onResetWeight }) {
  ...
  function handleIncomingWeight(raw) {
    ...
    const samples = [...weightSamplesRef.current, cleanWeight].slice(-4);
...
function InventoryReading({ disabled, row, products, selectedProduct, calculatedRemainingML, brandSearch, setBrandSearch, onPickProduct, closingFullBottle, setClosingFullBottle, closingEmptyBottle, setClosingEmptyBottle, closingOpenBottleMl, setClosingOpenBottleMl, onSave }) {
  const active = !!row || !!selectedProduct;
  const displayCategory = row?.category || selectedProduct?.category || "";
  const displaySize = row?.bottleSize || selectedProduct?.bottleSizeMl || "";
  const displayRemaining = closingOpenBottleMl || calculatedRemainingML || row?.closingOpenBottleRemainingMl || 0;
  return (
    <div className={`reading ${disabled ? "disabled" : ""}`}>
      <div className="readingTop">
        <div className="brandBlock">
          ...
          <p className="muted">{active ? `${displayCategory} • ${displaySize} ML` : "Select or scan a product"}</p>
        </div>
        <div className="readingRight">
          <div className="currentReading">
            <p className="muted">Current Reading</p>
            <p className="small">REMAINING</p>
            <h2>{active ? `${displayRemaining} ML` : "--/--"}</h2>
          </div>
        </div>
      </div>

      <div className="readingGrid">
        <div className="mini"><p>OPENING FULL BOTTLES</p><h2>{row ? row.openingFullBottleCount : "-"}</h2></div>
        <div className="mini"><p>OPENING OPEN BOTTLE ML</p><h2>{row ? row.openingOpenBottleRemainingMl : "-"}</h2></div>
...
function Dashboard({ onLogout }) {
  ...
  const selectedProduct = products.find((p) => p.id === selectedProductId) || products.find((p) => p.name === brandSearch);
  const calculatedRemainingML = selectedProduct && currentWeight
    ? Math.max(0, Math.min(
        Number(selectedProduct.bottleSizeMl || 0),
        Math.round(Number(currentWeight || 0) - Number(selectedProduct.emptyBottleWeightG || 0))
      ))
    : "";
  ...
  async function loadAll() {
    try {
      ...
      setRows((Array.isArray(stockData) ? stockData : []).map(mapStock));
    ...
    try {
      ...
      if (currentWeight) {
        const remaining = Math.max(
          0,
          Math.min(
            Number(product.bottleSizeMl || 0),
            Math.round(Number(currentWeight || 0) - Number(product.emptyBottleWeightG || 0))
          )
        );
  ...
  function handleWeightDetected(weight) {
    ...
    const product =
      ...
      products.find((p) => p.barcode === lastBarcode) ||
      products.find((p) => p.name === brandSearch);
    ...
    const remaining = Math.max(
      ...
      Math.min(
        Number(product.bottleSizeMl || 0),
        Math.round(Number(weight) - Number(product.emptyBottleWeightG || 0))
      )
    );
        ...
        {page === "dashboard" && (
          <>
            <section className="stats">
              ...
              />
              <StatCard title="Database Records" value={products.length} sub="Bottle masters" icon="🍾" />
            </section>
            ...
            <Card>
              ...
              <InventoryReading
                ...
                calculatedRemainingML={calculatedRemainingML}
